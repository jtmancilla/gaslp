...
for(j in 1:32){
  ...
  #Segundo bucle de extraccion por municipio
    for(k in 1:rela$Municipio[j]){
        ...
        esta<-substr(est,2,3)
        Sys.sleep(60*2)
        cre<- read_html(paste0("http://api-reportediario.cre.gob.mx/api/PlantaDistribucion/precio?localidadId=1&entidadId=",esta,"&municipioId=",muni))
        http://api-reportediario.cre.gob.mx/api/PlantaDistribucion/precio?localidadId=1121&entidadId=02&municipioId=001
        ...
        separa_2
        r<-separa_2[[1]]
        #Tercer bucle, estructuraci?n de la informacion
        for(i in 0:5000){
